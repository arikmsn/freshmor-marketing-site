...
const BENEFITS = [
  {
    ...
    accent: "border-r-4 border-brand-cyan",
    icon: (
  ...
  },
  {
    ...
    accent: "border-r-4 border-emerald-400",
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    accent: "border-r-4 border-sky-400",
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {BENEFITS.map((benefit, i) => (
            <motion.div
              key={benefit.title}
              className={`bg-white/5 border border-white/10 rounded-2xl p-6 cursor-default ${benefit.accent}`}
